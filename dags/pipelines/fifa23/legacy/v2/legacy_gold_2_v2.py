...
with DAG(
    dag_id="gold_legacy_pipeline_2_v2",
    schedule=None,
    start_date=datetime(2026, 6, 6),
    catchup=False,
    tags=["fifa23_legacy", "gold", "v2"],
) as dag:
    ...
    def create_tables_2():
        ...
        CREATE TABLE fifa23_gold.players_national_legacy (
            ...
            CONSTRAINT fk_players_legacy_nationality
        ...
        sql2 = """
        CREATE TABLE fifa23_gold.players_main_stats_legacy (
            player_id BIGINT NOT NULL CHECK (player_id > 0),
            fifa_version INT NOT NULL CHECK (fifa_version BETWEEN 6 AND 30),
            short_name VARCHAR(100) NOT NULL,

            preferred_foot VARCHAR(10) CHECK (
                preferred_foot IN ('Left', 'Right')
                OR preferred_foot IS NULL
            ),

            weak_foot INT CHECK (
                weak_foot BETWEEN 1 AND 5
                OR weak_foot IS NULL
            ),
            skill_moves INT CHECK (
                skill_moves BETWEEN 1 AND 5
            ...
            pace INT CHECK (pace BETWEEN 1 AND 99 OR pace IS NULL),
            shooting INT CHECK (shooting BETWEEN 1 AND 99 OR shooting IS NULL),
            passing INT CHECK (passing BETWEEN 1 AND 99 OR passing IS NULL),
            dribbling INT CHECK (dribbling BETWEEN 1 AND 99 OR dribbling IS NULL),
            defending INT CHECK (defending BETWEEN 1 AND 99 OR defending IS NULL),
            physic INT CHECK (physic BETWEEN 1 AND 99 OR physic IS NULL),
        ...
        sql3 = """
        CREATE TABLE fifa23_gold.players_aux_stats_legacy (
            player_id BIGINT NOT NULL CHECK (player_id > 0),
            fifa_version INT NOT NULL CHECK (fifa_version BETWEEN 6 AND 30),
            short_name VARCHAR(100) NOT NULL,
        ...
        sql4 = """
        CREATE TABLE fifa23_gold.players_legacy_fifaver (
            player_id BIGINT NOT NULL CHECK (player_id > 0),
            fifa_version INT NOT NULL CHECK (fifa_version BETWEEN 6 AND 30),
        ...
        sql5 = """
        CREATE TABLE fifa23_gold.players_legacy_for_debug (
            player_id BIGINT NOT NULL CHECK (player_id > 0),
            fifa_version INT NOT NULL CHECK (fifa_version BETWEEN 6 AND 30),
        ...
        sql6 = """
        CREATE VIEW fifa23_silver.latest_fifa23_legacy_2 AS
        SELECT *
        FROM (
            SELECT
                *,
            ...
            FROM fifa23_silver.silver_players_legacy
            WHERE fifa_version BETWEEN 15 AND 18
        ) x
        WHERE rn = 1;
        """

        with engine.begin() as conn:
            conn.execute(text("""DROP VIEW IF EXISTS fifa23_silver.latest_fifa23_legacy_2;"""))
            conn.execute(text("""DROP TABLE IF EXISTS fifa23_gold.players_national_legacy CASCADE;"""))
            conn.execute(text("""DROP TABLE IF EXISTS fifa23_gold.players_main_stats_legacy CASCADE;"""))
            conn.execute(text("""DROP TABLE IF EXISTS fifa23_gold.players_aux_stats_legacy CASCADE;"""))
            conn.execute(text("""DROP TABLE IF EXISTS fifa23_gold.players_legacy_fifaver CASCADE;"""))
            conn.execute(text("""DROP TABLE IF EXISTS fifa23_gold.players_legacy_for_debug CASCADE;"""))

            conn.execute(text(sql6))
    ...
    def insert_into_players_national_legacy():
        ...
        sql = """
        INSERT INTO fifa23_gold.players_national_legacy (
            player_id,
            fifa_version,
            short_name,
            nationality_id,
        ...
        SELECT
            ...
            nation_jersey_number
        FROM fifa23_silver.latest_fifa23_legacy_2
    ...
    def insert_into_players_main_stats_legacy():
        ...
        sql = """
        INSERT INTO fifa23_gold.players_main_stats_legacy (
            player_id,
            fifa_version,
            short_name,
            preferred_foot,
    ...
    def insert_into_players_aux_stats_legacy():
        ...
        sql = """
        INSERT INTO fifa23_gold.players_aux_stats_legacy (
            player_id,
            fifa_version,
            short_name,
    ...
    def insert_into_players_legacy_fifaver():
        ...
        sql = """
        INSERT INTO fifa23_gold.players_legacy_fifaver (
            player_id,
            fifa_version,
            fifa_update,
            fifa_update_date,
    ...
    def insert_into_players_legacy_for_debug():
        ...
        sql = """
        INSERT INTO fifa23_gold.players_legacy_for_debug (
            player_id,
            fifa_version,
            source_file,
            chunk_no
        ...
        FROM fifa23_silver.latest_fifa23_legacy_2
        WHERE player_id IS NOT NULL
          AND fifa_version BETWEEN 15 AND 18;
        """
